...
def get_embs(seed, temp, model, df, text, split, bsize, start, proxy):
    ...
    for x in tqdm(samples, desc= model+" embeddings"):
        array = get_ollama_emb(prompt=x, seed=seed, temp=temp, model=model)
        tensors.append(array)
    tensors = torch.stack(tensors, dim=0).to(device)
    print(tensors.shape)
    torch.save(tensors, './/MISTRAL//'+model+f'_syn_score_{split}_batch{str(start)}.pt')
...
for x in tqdm(range(bstart_train, math.ceil(len(train_df)/bsize)), desc="Batch"):
    get_embs(seed=42, temp=0.2, model="mistral", df=train_df, text="Synopsis", split="train", bsize=bsize, start=x, proxy=your_proxy)

bstart_test = 0
for x in tqdm(range(bstart_test, math.ceil(len(test_df)/bsize)), desc="Batch"):
    get_embs(seed=42, temp=0.2, model="mistral", df=test_df, text="Synopsis", split="test", bsize=bsize, start=x, proxy=your_proxy)
